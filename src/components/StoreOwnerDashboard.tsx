...
import {
  ...
  DollarSign, ShoppingBag, Power, Loader2, Clock, MapPin, Phone, Save, Upload,
...
export function StoreOwnerDashboard({ storeId, storeName, onLogout }: Props) {
  ...
  if (loading) return (
    <div className="min-h-screen w-full flex items-center justify-center bg-white text-gray-900">
  ...
  return (
    <div className="min-h-screen w-full bg-white text-gray-900 lg:flex">
      ...
      <div className="lg:hidden sticky top-0 z-50 bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3">
        ...
        <p className="flex-1 text-sm font-black text-gray-900 truncate">{store?.store_name || storeName}</p>
        <button onClick={toggleAccepting} className={`text-[9px] font-black px-2.5 py-1 rounded-full border shrink-0 ${(store?.accepting_orders ?? true) ? 'bg-green-50 text-green-600 border-green-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
      ...
      <aside className={`fixed top-0 bottom-0 left-0 z-50 w-60 shrink-0 bg-white border-r border-gray-200 flex flex-col transition-transform duration-300 lg:static lg:translate-x-0 lg:h-screen lg:sticky lg:top-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center gap-3 px-5 py-5 border-b border-gray-200">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-black text-gray-900 truncate">{store?.store_name || storeName}</p>
            <p className="text-[9px] text-orange-500 font-black uppercase tracking-widest">Store Dashboard</p>
          </div>
        ...
        <div className="p-3 border-t border-gray-200 space-y-1">
          <button onClick={toggleAccepting} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-black border transition-colors ${store?.accepting_orders ? 'bg-green-50 text-green-600 border-green-200 hover:bg-green-100/50' : 'bg-gray-100 text-gray-500 border-gray-200 hover:bg-gray-200/50'}`}>
            <span className="flex items-center gap-2">
              <Power className="w-3.5 h-3.5" /> 
              {store?.accepting_orders ? 'Accepting Orders' : 'Orders Paused'}
            </span>
            {store?.accepting_orders ? (
              <ToggleRight className="w-6 h-6 text-green-600 shrink-0" />
            ) : (
              <ToggleLeft className="w-6 h-6 text-gray-400 shrink-0" />
            )}
          </button>
          <button onClick={onLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold text-gray-700 hover:text-red-600 hover:bg-red-50 transition-colors">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-stone-900 text-white text-sm font-bold px-5 py-3 rounded-2xl shadow-2xl">
            {toast}
          </motion.div>
        )}
      </AnimatePresence>
...
function Overview({ store, menu, orders, onGoTab, canAccessFinancials }: { store: StoreDoc | null; menu: MenuItemDoc[]; orders: any[]; onGoTab: (t: Tab) => void; canAccessFinancials: boolean }) {
  ...
  return (
    <div>
      ...
      {pending.length > 0 && (
        <button onClick={() => onGoTab('orders')} className="w-full mb-6 bg-orange-50 border border-orange-200 rounded-2xl px-5 py-4 flex items-center justify-between text-left hover:border-orange-350 transition-colors shadow-sm">
      ...
      {orders.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center text-gray-500 text-sm shadow-sm">No orders yet.</div>
      ) : (
        <div className="space-y-2">
          {orders.slice(0, 5).map(o => (
            <div key={o.id} className="bg-white border border-gray-200 rounded-2xl px-5 py-3 flex items-center justify-between shadow-sm">
              <div>
                <p className="text-sm font-bold text-gray-900">{(o.items?.[0]?.pizzaName ?? o.items?.[0]?.name) || 'Order'}{o.items?.length > 1 ? ` +${o.items.length - 1}` : ''}</p>
                <p className="text-[11px] text-gray-600 capitalize">{(o.orderStatus ?? o.status ?? 'placed').replace(/_/g, ' ')}</p>
              </div>
              <p className="font-black text-gray-900">{money(o.finalTotal ?? o.total)}</p>
            </div>
          ))}
        </div>
      )}
...
function MenuManager({ storeId, menu, reload, flash, setMenu }: {
  ...
  const openCamera = async () => {
    ...
    try {
      ...
      setScanPhase('camera');
      setTimeout(() => { if (videoRef.current) { videoRef.current.srcObject = stream; videoRef.current.play().catch(() => {}); } }, 55);
    } catch (err: any) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        setScannedItems(data.map((item: any) => ({ ...item, checked: true })));
        setScanPct(100);
        setScanPhase('done');
      } catch (err) {
        clearInterval(progressInterval);
        console.error(err);
        setCamError('Failed to parse menu image. Please try again.');
        setScanPhase('idle');
      }
    };
  ...
  const startScan = async () => {
    ...
    if (!base64Image) {
      setCamError('Failed to capture image. Point camera and try again.');
      setScanPhase('camera');
      return;
    }
    ...
    try {
      ...
      clearInterval(progressInterval);
      console.error(err);
      setCamError('OCR failed. Please check your image or try uploading a file.');
      setScanPhase('idle');
    }
  };

  const importScanned = async () => {
    const itemsToImport = scannedItems.filter(it => it.checked);
    if (itemsToImport.length === 0) {
      flash('No items selected for import');
      return;
    }
    setImporting(true);
    try {
      ...
      await reload();
      flash(`Menu imported — ${itemsToImport.length} items added!`);
      setShowScan(false);
      setScanPhase('idle');
    } catch (e) {
      console.error(e);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">{menu.length} item{menu.length !== 1 ? 's' : ''} · saves instantly</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => { setShowScan(true); setScanPhase('idle'); setScanPct(0); }}
            className="inline-flex items-center gap-2 bg-violet-50 hover:bg-violet-100 border border-violet-200 text-violet-600 text-sm font-bold px-4 py-2.5 rounded-xl transition-all">
            <Camera className="w-4 h-4" /> Import Menu
          </button>
          <button onClick={() => setForm({ ...emptyItem })}
            className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-red-600 text-white text-sm font-black px-4 py-2.5 rounded-xl transition-all">
      ...
      {menu.length === 0 && !form && (
        <div className="bg-violet-50 border border-violet-150 rounded-2xl p-5 flex gap-4 items-start mb-6">
          <Camera className="w-6 h-6 text-violet-500 shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-black text-violet-700 mb-1">Import your menu in seconds</p>
            <p className="text-xs text-gray-500 leading-relaxed mb-3">Point your camera at any printed or digital menu. MiSlice AI extracts all items, categories, and prices automatically.</p>
                ...
                {priceEdit?.id === it.id ? (
                  <div className="flex items-center gap-1">
                    ...
                    <input autoFocus type="number" step="0.01" value={priceEdit.value}
                      ...
                      className="w-20 bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 text-sm text-gray-950 focus:outline-none focus:border-red-500" />
                    <button onClick={() => savePrice(it.id)} className="text-green-600 p-1"><Check className="w-4 h-4" /></button>
                    <button onClick={() => setPriceEdit(null)} className="text-gray-400 p-1"><X className="w-4 h-4" /></button>
                  </div>
                ) : (
                  <button onClick={() => setPriceEdit({ id: it.id, value: String(it.price) })}
                    className="text-sm font-black text-gray-950 hover:text-orange-500 transition-colors flex items-center gap-1 group">
                    {money(it.price)} <Edit2 className="w-3 h-3 opacity-0 group-hover:opacity-100 text-gray-400" />
                  </button>
                )}
                <button onClick={() => toggleAvail(it)} className={`text-[9px] font-black px-2 py-1 rounded-full border ${it.available ? 'bg-green-50 text-green-600 border-green-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                  {it.available ? 'LIVE' : 'OFF'}
                </button>
                <button onClick={() => setForm({ ...it })} className="text-gray-400 hover:text-gray-900 p-1"><Edit2 className="w-4 h-4" /></button>
                <button onClick={() => remove(it.id)} className="text-gray-400 hover:text-red-500 p-1"><Trash2 className="w-4 h-4" /></button>
              </div>
            ))}
          </div>
        </div>
      ...
      <AnimatePresence>
        {showScan && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeScan} className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl">
              <div className="flex justify-between items-center mb-5">
                ...
                <button onClick={closeScan} className="text-gray-400 hover:text-gray-700"><X className="w-5 h-5" /></button>
              </div>

              {scanPhase === 'idle' && (
                <>
                  <div className="bg-gray-50 border border-gray-250 rounded-2xl aspect-video flex flex-col items-center justify-center gap-3 mb-5">
                    <Camera className="w-12 h-12 text-gray-300" />
                    <p className="text-xs font-bold text-gray-500 text-center px-4">MiSlice AI will scan your physical or digital menu and extract all items, categories, and prices automatically.</p>
                    {camError && <p className="text-xs text-red-500 font-bold text-center px-4">{camError}</p>}
                  </div>
                  ...
                  <label
                    ...
                    className="w-full py-3 border border-dashed border-gray-300 hover:border-violet-500 rounded-xl flex items-center justify-center gap-2 text-sm font-bold text-gray-600 hover:text-violet-600 cursor-pointer transition-all bg-gray-50/50 hover:bg-violet-50/10"
                  >
                    <Upload className="w-4 h-4" /> Upload Menu Image
                  </label>
                </>
              )}
                ...
                <>
                  <div className="relative rounded-2xl overflow-hidden aspect-video mb-5 bg-black">
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                      ...
                      <div className="absolute left-4 right-4 h-0.5 bg-gradient-to-r from-transparent via-violet-450 to-transparent animate-[scan_2s_ease-in-out_infinite]" style={{ top: '55%' }} />
                      <div className="absolute bottom-3 left-0 right-0 text-center">
                        <span className="text-[10px] font-black text-violet-200 bg-black/60 px-2 py-0.5 rounded-full">Point camera at your menu</span>
                      </div>
                    </div>
                  </div>
              ...
              {scanPhase === 'scanning' && (
                <div className="py-8 text-center space-y-5">
                  <div className="w-16 h-16 mx-auto bg-violet-50 border border-violet-100 rounded-2xl flex items-center justify-center">
                  ...
                  <p className="text-xs font-black text-violet-600">{scanPct}%</p>
                </div>
              )}

              {scanPhase === 'done' && (
                <div className="py-2 text-center space-y-4">
                  <div className="w-12 h-12 mx-auto bg-green-50 border border-green-150 rounded-2xl flex items-center justify-center">
                    <CheckCircle2 className="w-6 h-6 text-green-500" />
                  </div>
                  <div>
                    <p className="text-sm font-black text-gray-900">Scan Complete!</p>
                    <p className="text-xs text-gray-500 mt-0.5">Found {scannedItems.length} items. Select items to import:</p>
                  </div>
                  
                  {/* Scrollable list of scanned items with editable fields */}
                  <div className="max-h-80 overflow-y-auto border border-gray-200 rounded-2xl bg-gray-50/50 p-2.5 space-y-2 text-left">
                    {scannedItems.map((item, index) => (
                      <div key={index} className="bg-white border border-gray-150 rounded-xl p-3 space-y-2">
                        <div className="flex items-start gap-2.5">
                          <input
                            type="checkbox"
                            checked={item.checked ?? true}
                            onChange={e => {
                              setScannedItems(prev => prev.map((it, idx) => idx === index ? { ...it, checked: e.target.checked } : it));
                            }}
                            className="mt-1.5 accent-violet-600 shrink-0"
                          />
                          <div className="flex-1 space-y-2">
                            ...
                            <input
                              ...
                              value={item.name}
                              onChange={e => {
                                setScannedItems(prev => prev.map((it, idx) => idx === index ? { ...it, name: e.target.value } : it));
                              }}
                              className="w-full bg-gray-50 border border-gray-200 rounded-lg px-2.5 py-1 text-xs font-bold text-gray-900 focus:outline-none focus:border-violet-500"
                            ...
                            <input
                              ...
                              onChange={e => {
                                setScannedItems(prev => prev.map((it, idx) => idx === index ? { ...it, description: e.target.value } : it));
                              }}
                              className="w-full bg-gray-50 border border-gray-200 rounded-lg px-2.5 py-1 text-[10px] text-gray-600 focus:outline-none focus:border-violet-500"
                              placeholder="Description"
                            />

                            <div className="flex gap-2">
                              {/* Price Input */}
                              <div className="flex-1 flex items-center bg-gray-50 border border-gray-200 rounded-lg px-2 text-[10px]">
                                ...
                                <input
                                  ...
                                  value={item.price}
                                  onChange={e => {
                                    setScannedItems(prev => prev.map((it, idx) => idx === index ? { ...it, price: Number(e.target.value) || 0 } : it));
                                  }}
                                  className="w-full bg-transparent px-1 py-1 text-gray-900 outline-none font-bold"
                                  placeholder="0.00"
                                />
                              </div>
                              ...
                              <select
                                ...
                                onChange={e => {
                                  setScannedItems(prev => prev.map((it, idx) => idx === index ? { ...it, category: e.target.value } : it));
                                }}
                                className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 text-[10px] text-gray-900 focus:outline-none"
                              >
                  ...
                  <div className="flex gap-3">
                    <button onClick={closeScan} className="flex-1 py-2.5 rounded-xl text-sm font-bold text-gray-500 bg-gray-100 hover:bg-gray-200 transition-colors">Cancel</button>
                    <button onClick={importScanned} disabled={importing} className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white bg-green-600 hover:bg-green-700 transition-all flex items-center justify-center gap-2">
                      {importing ? <Loader2 className="w-4 h-4 animate-spin" /> : `Import Selected (${scannedItems.filter(it => it.checked).length})`}
                    </button>
                  </div>
                </div>
              )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl">
              ...
              <div className="space-y-3">
                <input placeholder="Item name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-950 text-sm focus:outline-none focus:border-red-500" />
                <input placeholder="Description" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-950 text-sm focus:outline-none focus:border-red-500" />
                <div className="flex gap-3">
                  <div className="flex-1 flex items-center bg-gray-50 border border-gray-200 rounded-xl px-3">
                    <span className="text-gray-400 text-sm">$</span>
                    <input type="number" step="0.01" placeholder="0.00" value={form.price || ''} onChange={e => setForm({ ...form, price: e.target.value })} className="w-full bg-transparent px-2 py-3 text-gray-955 text-sm outline-none" />
                  </div>
                  <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-gray-955 text-sm">
                    {CATEGORIES.map(c => <option key={c} value={c} className="bg-white text-gray-955">{c}</option>)}
                  </select>
                </div>
              ...
              <div className="flex gap-3 mt-5">
                <button onClick={() => setForm(null)} className="flex-1 py-3 rounded-xl text-sm font-bold bg-gray-100 text-gray-500 hover:bg-gray-200">Cancel</button>
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500">Published deals appear on the MiSlice customer Deals page.</p>
        </div>
        <button onClick={() => setForm({ ...emptyDeal })} className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-red-600 text-white text-sm font-black px-4 py-2.5 rounded-xl transition-all">
          <Plus className="w-4 h-4" /> New Deal
        </button>
      </div>

      <div className="mb-6">
        <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-3">Quick Templates</p>
        <div className="flex flex-wrap gap-2">
          {DEAL_TEMPLATES.map(t => (
            <button key={t.label} onClick={() => setForm({ ...t })}
              className="flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl text-sm font-bold text-gray-700 hover:text-black transition-all">
              {t.emoji} {t.label}
            </button>
          ))}
        </div>
      ...
      {deals.length === 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center text-gray-500 text-sm shadow-sm">No deals yet. Create one or pick a template above.</div>
      )}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <div className="flex items-center bg-gray-50 border border-gray-200 rounded-lg px-2.5 py-1">
                      <span className="text-[10px] font-black text-gray-700 font-mono tracking-wider">{code}</span>
                    </div>
                    <button onClick={() => copyCode(code)} className="text-gray-400 hover:text-gray-900 transition-colors">
                      {copiedId === code ? <CheckCircle2 className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                </div>
                {!!d.discounted_price && (
                  <p className="text-sm font-black text-green-600 shrink-0">{money(d.discounted_price)}{!!d.original_price && <span className="text-gray-400 line-through ml-1 text-xs">{money(d.original_price)}</span>}</p>
                )}
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => toggle(d)}>
                    {d.is_active ? <ToggleRight className="w-6 h-6 text-green-500" /> : <ToggleLeft className="w-6 h-6 text-gray-300" />}
                  </button>
                  <button onClick={() => setForm({ ...d })} className="text-gray-400 hover:text-gray-900 p-1"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => remove(d.id)} className="text-gray-400 hover:text-red-500 p-1"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          );
        ...
        {form && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl">
              ...
              <div className="space-y-3">
                <input placeholder="Deal title" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-955 text-sm focus:outline-none focus:border-red-500" />
                <textarea placeholder="Description" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={2} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-955 text-sm resize-none focus:outline-none focus:border-red-500" />
                <div className="flex gap-3">
                  <div className="flex-1 flex items-center bg-gray-50 border border-gray-200 rounded-xl px-3">
                    <span className="text-gray-400 text-xs">Was $</span>
                    <input type="number" step="0.01" value={form.original_price || ''} onChange={e => setForm({ ...form, original_price: e.target.value })} className="w-full bg-transparent px-2 py-3 text-gray-955 text-sm outline-none" />
                  </div>
                  <div className="flex-1 flex items-center bg-gray-50 border border-gray-200 rounded-xl px-3">
                    <span className="text-gray-400 text-xs">Now $</span>
                    <input type="number" step="0.01" value={form.discounted_price || ''} onChange={e => setForm({ ...form, discounted_price: e.target.value })} className="w-full bg-transparent px-2 py-3 text-gray-955 text-sm outline-none" />
                  </div>
                </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-0 sm:px-4">
      ...
      <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
        className="relative w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl pb-safe overflow-hidden shadow-2xl border border-gray-200">

        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-200">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-500">Live Tracking</p>
            <p className="text-base font-black text-gray-900">Order #{String(order.id).slice(-6).toUpperCase()}</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-gray-150 text-gray-400 hover:text-gray-700"><X className="w-5 h-5" /></button>
        </div>

        <div className="p-5 space-y-5">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-4">Order Progress</p>
                ...
                return (
                  <div key={i} className="flex gap-3 items-start">
                    <div className="flex flex-col items-center">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 border-2 transition-all ${done ? 'bg-green-500 border-green-500' : current ? 'bg-orange-500 border-orange-500 animate-pulse' : 'bg-transparent border-gray-200'}`}>
                        {done ? <Check className="w-3.5 h-3.5 text-white" /> : current ? <div className="w-2 h-2 bg-white rounded-full" /> : <div className="w-2 h-2 bg-gray-300 rounded-full" />}
                      </div>
                      {i < 5 && <div className={`w-0.5 h-5 ${i < stageIdx ? 'bg-green-500' : 'bg-gray-100'}`} />}
                    </div>
                    <p className={`text-sm font-bold pt-1 pb-4 ${done ? 'text-green-600' : current ? 'text-orange-500' : 'text-gray-400'}`}>{label}</p>
                  </div>
                );
              })}
            </div>
          ...
          <div className="rounded-2xl overflow-hidden h-36 bg-gray-50 border border-gray-200 relative flex items-center justify-center">
            ...
            {showDriver ? (
              ...
              <div className="relative z-10 text-center">
                <Navigation className="w-6 h-6 text-gray-350 mx-auto mb-1" />
          ...
          {showDriver ? (
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-3">Delivery Partner</p>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-gray-500 truncate">{driver.vehicle}</p>
                </div>
              ...
              <div className="mt-3 flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-3 py-2">
                ...
                <p className="text-[10px] text-gray-500 ml-auto">{driver.phone}</p>
              </div>
            </div>
          ) : (
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 flex items-center gap-3">
              <User className="w-5 h-5 text-gray-450 shrink-0" />
              <div>
                <p className="text-sm font-bold text-gray-700">Driver not assigned yet</p>
                <p className="text-xs text-gray-500">A delivery partner will be assigned once the order is ready for pickup.</p>
              </div>
            </div>
          )}
        </div>
...
function Orders({ orders, flash, canUpdateOrders, canAccessFinancials }: { orders: any[]; flash: (m: string) => void; canUpdateOrders: boolean; canAccessFinancials: boolean }) {
  ...
  return (
    <div>
      ...
      {view === 'active' && (
        active.length === 0 ? <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center text-gray-500 text-sm shadow-sm">No active orders.</div> :
        <div className="space-y-3">
          {active.sort((a,b) => String(b.createdAt ?? '').localeCompare(String(a.createdAt ?? ''))).map(o => {
            ...
            return (
              <div key={o.id} className={`bg-white border border-gray-200 rounded-2xl p-5 shadow-sm ${isNew ? 'border border-orange-500/50 bg-orange-50/20' : ''}`}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-[11px] text-gray-500">{o.createdAt ? new Date(o.createdAt).toLocaleString() : ''} · {o.deliveryType || 'delivery'}{o.prepMinutes ? ` · ${o.prepMinutes}min prep` : ''}</p>
                  </div>
                  <p className="font-black text-gray-900">{money(o.finalTotal ?? o.total)}</p>
                </div>
                <div className="text-xs text-gray-600 mb-3 space-y-0.5 font-medium">
                ...
                {!canUpdateOrders ? (
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {PREP_OPTIONS.map(p => (
                      <button key={p} onClick={() => accept(o, p)} className="text-[11px] font-bold bg-green-50 text-green-600 border border-green-200 hover:bg-green-100/50 px-2.5 py-1.5 rounded-lg transition-colors">{p}m</button>
                    ))}
                    <button onClick={() => advance(o, 'cancelled')} className="ml-auto text-[11px] font-bold text-red-500 hover:bg-red-55/10 border border-red-200 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1 bg-white"><X className="w-3 h-3" /> Reject</button>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 flex-wrap">
                    <button onClick={() => setTrackingOrder(o)} className="flex items-center gap-1.5 text-[11px] font-bold bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-100 transition-colors px-3 py-1.5 rounded-lg">
                      <Navigation className="w-3 h-3" /> Track
                    </button>
                    <span className="text-[10px] font-black uppercase tracking-wider text-orange-500 font-bold">{status.replace(/_/g, ' ')}</span>
                    <select value={status} onChange={e => advance(o, e.target.value)} className="ml-auto bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-700 font-bold">
                      {ORDER_STAGES.map(s => <option key={s} value={s} className="bg-white text-gray-950">{s.replace(/_/g, ' ')}</option>)}
                    </select>
                  </div>
                )}
              </div>
      ...
      {view === 'past' && (
        past.length === 0 ? <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center text-gray-500 text-sm shadow-sm">No past orders.</div> :
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm divide-y divide-gray-150">
          <div className="grid grid-cols-[1fr_80px_100px_90px] text-[9px] font-black uppercase tracking-widest text-gray-500 px-5 py-3 bg-gray-50">
          ...
          {past.map(o => (
            <div key={o.id} className="grid grid-cols-[1fr_80px_100px_90px] items-center px-5 py-3 hover:bg-gray-55/50 transition-colors">
              <div>
                <p className="text-xs font-black text-gray-900">#{String(o.id).slice(-6).toUpperCase()}</p>
                <p className="text-[9px] text-gray-400">{o.createdAt ? new Date(o.createdAt).toLocaleDateString() : ''}</p>
              </div>
              <p className="text-xs text-gray-500 text-center">{(o.items||[]).length}</p>
              <p className="text-sm font-black text-gray-900 text-center">{money(o.finalTotal ?? o.total)}</p>
              <p className={`text-[9px] font-black uppercase text-center ${(o.orderStatus ?? o.status) === 'delivered' ? 'text-green-600' : 'text-red-500'}`}>{(o.orderStatus ?? o.status ?? '').replace(/_/g,' ')}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {view === 'receipts' && (
        <div className="space-y-3">
          <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-4 shadow-sm">
            <p className="text-xs font-black text-gray-500 mb-3 uppercase tracking-widest">Commission Summary (All Time)</p>
            {(() => {
              ...
              return (
                <div className="flex gap-6">
                  <div><p className="text-[9px] text-gray-400 font-bold uppercase">Gross</p><p className="text-lg font-black text-gray-900">{money(gross)}</p></div>
                  <div><p className="text-[9px] text-gray-400 font-bold uppercase">MiSlice 20%</p><p className="text-lg font-black text-red-500">−{money(fee)}</p></div>
                  <div><p className="text-[9px] text-gray-400 font-bold uppercase">Net Earnings</p><p className="text-lg font-black text-green-600">{money(net)}</p></div>
                </div>
              );
            })()}
          </div>
          <div className="bg-white border border-gray-200 rounded-2xl divide-y divide-gray-150 overflow-hidden shadow-sm">
            <div className="grid grid-cols-[1fr_100px_100px_100px] text-[9px] font-black uppercase tracking-widest text-gray-500 px-5 py-3 bg-gray-50">
              <span>Order</span><span className="text-center">Gross</span><span className="text-center">Fee (20%)</span><span className="text-center">Payout</span>
            </div>
            {orders.filter(o => (o.orderStatus ?? o.status) !== 'cancelled').map(o => {
              const amt = Number(o.finalTotal ?? o.total) || 0;
              const fee = amt * PLATFORM_FEE;
              const pay = amt - fee;
              return (
                <div key={o.id} className="grid grid-cols-[1fr_100px_100px_100px] items-center px-5 py-3 hover:bg-gray-55/50 transition-colors">
                  <div>
                    <p className="text-xs font-black text-gray-950">#{String(o.id).slice(-6).toUpperCase()}</p>
                    <p className="text-[9px] text-gray-400">{o.createdAt ? new Date(o.createdAt).toLocaleDateString() : ''}</p>
                  </div>
                  <p className="text-sm font-bold text-gray-900 text-center">{money(amt)}</p>
                  <p className="text-sm font-bold text-red-500 text-center">−{money(fee)}</p>
                  <p className="text-sm font-black text-green-600 text-center">{money(pay)}</p>
                </div>
              );
            })}
          </div>
...
function AIInsights({ orders }: { orders: any[] }) {
  ...
  Object.entries(thisItems).forEach(([name, cnt]) => {
    ...
    if (prev > 0 && cnt > prev) {
      ...
      if (pct >= 30) insights.push({ id: `up_${name}`, cat: 'sales', icon: TrendingUp, color: 'text-green-600', bg: 'bg-green-50 border border-green-150', title: `${name} orders up ${pct}% this week`, detail: `${cnt} orders vs ${prev} last week. Consider featuring it as a deal to capitalise on demand.` });
    }
    if (prev > 0 && cnt < prev) {
      const pct = Math.round(((prev - cnt) / prev) * 100);
      if (pct >= 30) insights.push({ id: `dn_${name}`, cat: 'sales', icon: TrendingDown, color: 'text-red-500', bg: 'bg-red-55/10 border border-red-150', title: `${name} orders dropped ${pct}%`, detail: `Only ${cnt} orders vs ${prev} last week. Try a limited-time discount to bring customers back.` });
    }
  });
  ...
  if (days.length >= 2) {
    ...
    if (+peak[1] > +slow[1] * 1.4) {
      ...
      insights.push({ id: 'peak_day', cat: 'operations', icon: Clock, color: 'text-yellow-600', bg: 'bg-yellow-50 border border-yellow-150', title: `${dayNames[+peak[0]]} revenue is ${pct}% higher than ${dayNames[+slow[0]]}`, detail: `Consider running a promotion on ${dayNames[+slow[0]]} to boost your slowest day.` });
    }
  }

  const cancelRate = orders.length ? (orders.filter(o => (o.orderStatus ?? o.status) === 'cancelled').length / orders.length) * 100 : 0;
  if (cancelRate > 10) insights.push({ id: 'cancel', cat: 'operations', icon: TrendingDown, color: 'text-red-500', bg: 'bg-red-55/10 border border-red-150', title: `High cancellation rate — ${cancelRate.toFixed(0)}%`, detail: 'Review your prep time estimates. High cancellations can hurt your MiSlice ranking.' });

  insights.push(
    { id: 'family_deal', cat: 'suggestions', icon: Lightbulb, color: 'text-violet-600', bg: 'bg-violet-50 border border-violet-150', title: 'Add a Family Meal Deal to boost average order value', detail: 'Bundles (2 large pizzas + drinks) increase AOV by 35–60%. Customers love them on weekends.' },
    { id: 'drinks_upsell', cat: 'suggestions', icon: Lightbulb, color: 'text-blue-600', bg: 'bg-blue-50 border border-blue-150', title: 'Pair beverages with pizza at checkout', detail: 'Customers who add a drink spend 28% more on average. Add a "Add a drink?" prompt in your deals.' },
    { id: 'photos', cat: 'suggestions', icon: Camera, color: 'text-orange-600', bg: 'bg-orange-50 border border-orange-150', title: 'Add food photos to your top 5 menu items', detail: 'Listings with photos get 3× more clicks on MiSlice. Upload high-quality images to your menu.' },
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2"><Zap className="w-6 h-6 text-yellow-500" /> AI Insights</h1>
        <button onClick={() => { setRefreshing(true); setTimeout(() => setRefreshing(false), 1000); }} className="flex items-center gap-1.5 text-xs font-bold text-gray-500 hover:text-black px-3 py-2 bg-white border border-gray-200 rounded-xl transition-all">
          <RefreshCw className={`w-3.5 h-3.5 ${refreshing ? 'animate-spin' : ''}`} /> Refresh
        </button>
      </div>
      <p className="text-sm text-gray-500 mb-6">Real-time intelligence about your store performance and customers.</p>

      <div className="flex gap-2 flex-wrap mb-6">
        {(['all','sales','operations','suggestions'] as InsightFilter[]).map(f => (
          <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-xl text-xs font-bold capitalize transition-all ${filter === f ? 'bg-red-600 text-white shadow-sm' : 'bg-white border border-gray-200 text-gray-500 hover:text-gray-900'}`}>{f}</button>
        ))}
      </div>

      {shown.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center text-gray-500 text-sm shadow-sm">No insights yet. Insights grow smarter as orders come in.</div>
      ) : (
        <div className="space-y-3">
          {shown.map(ins => {
            const Icon = ins.icon;
            return (
              <div key={ins.id} className={`${ins.bg} rounded-2xl p-4 flex gap-4 shadow-sm`}>
                <div className={`w-9 h-9 rounded-xl bg-white border border-gray-150 flex items-center justify-center shrink-0`}>
                  <Icon className={`w-4.5 h-4.5 ${ins.color}`} />
                </div>
                <div>
                  <p className="text-sm font-black text-gray-900 mb-0.5">{ins.title}</p>
                  <p className="text-xs text-gray-500 leading-relaxed font-medium">{ins.detail}</p>
                </div>
              </div>
            );
          })}
...
function Earnings({ orders }: { orders: any[] }) {
  ...
  const filtered = filter(orders);
  const gross    = filtered.reduce((s, o) => s + (Number(o.finalTotal ?? o.total) || 0), 0);
  const fee      = gross * PLATFORM_FEE;
  const net      = gross - fee;
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500">Track payouts, sales volume, and marketplace commission fees.</p>
        </div>
        <div className="flex gap-1 bg-gray-100 border border-gray-250 p-1 rounded-xl">
          {(['daily','weekly','monthly','yearly'] as EarningsPeriod[]).map(p => (
            <button key={p} onClick={() => setPeriod(p)} className={`px-2.5 py-1.5 rounded-lg text-xs font-black capitalize transition-all ${period === p ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-955'}`}>{p}</button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Gross Volume', value: gross, color: 'text-gray-900' },
          { label: 'MiSlice 20% Fee',  value: fee,   color: 'text-red-500'  },
          { label: 'Net Payout',   value: net,   color: 'text-green-600' },
        ].map(card => (
          <div key={card.label} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
            ...
            <p className={`text-2xl font-black ${card.color}`}>{money(card.value)}</p>
          </div>
        ))}
      </div>

      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
        <p className="text-xs font-black text-gray-700 uppercase tracking-widest mb-4">Payout Schedule</p>
        <div className="flex gap-4 items-start text-xs text-gray-500">
          <ShieldCheck className="w-5 h-5 text-green-600 shrink-0 mt-0.5" />
          <div className="space-y-1 font-medium">
            <p className="text-gray-800 font-bold">Automatic payouts are set up</p>
            <p>Your net earnings are automatically deposited to your linked bank account every Monday morning (EST) at 5:00 AM.</p>
          </div>
        </div>
      </div>
    </div>
