  ...
  template: `
    <div class="max-w-6xl mx-auto py-8 space-y-8">
      ...
      <div *ngIf="selectedShop()" class="space-y-6 animate-fadeIn">
        <!-- KPI cards (React store-owner overview parity) -->
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div class="glass rounded-2xl p-5" style="border-color: color-mix(in srgb,#60A5FA 30%,transparent)">
            <span class="text-lg">📦</span>
            <p class="text-2xl font-black text-white mt-1">{{ orders().length }}</p>
            <p class="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-0.5">Total Orders</p>
          </div>
          <div class="glass rounded-2xl p-5" style="border-color: color-mix(in srgb,#34D399 30%,transparent)">
            <span class="text-lg">💰</span>
            <p class="text-2xl font-black text-white mt-1">{{ netEarnings() | currency }}</p>
            <p class="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-0.5">Net Earnings</p>
          </div>
          <div class="glass rounded-2xl p-5" style="border-color: color-mix(in srgb,#FBBF24 30%,transparent)">
            <span class="text-lg">🔥</span>
            <p class="text-2xl font-black text-white mt-1">{{ activeOrders() }}</p>
            <p class="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-0.5">Active Now</p>
          </div>
          <div class="glass rounded-2xl p-5" style="border-color: color-mix(in srgb,#FF6B35 30%,transparent)">
            <span class="text-lg">🍕</span>
            <p class="text-2xl font-black text-white mt-1">{{ menuCount() }}</p>
            <p class="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-0.5">Menu Items</p>
          </div>
        </div>
...
export class OwnerDashboardComponent implements OnInit {
  ...
  menuCount = computed(() => this.selectedShop() ? '—' : '0');
